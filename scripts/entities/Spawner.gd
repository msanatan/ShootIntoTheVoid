...
func create_instance(entity_scene, rotate_to_player):
	...
	if next_loc.x != -1 && next_loc.y != -1:
		...
		if rotate_to_player:
			entity.rotation = (
				(player_scene.global_position - entity.global_position).angle()
				+ PI / 2
			)
		add_child(entity)
		return entity
	...
	while attempt_count < max_attempts:
		...
		for loc in spawn_locs:
			var overlap = is_overlapping(
				x, y, w, h, loc.location.x, loc.location.y, loc.width, loc.height
			)
			if overlap:
				too_close = true
				break
		...
		if !too_close:
			spawn_locs.append({location = new_loc, width = w, height = h})
			return new_loc
